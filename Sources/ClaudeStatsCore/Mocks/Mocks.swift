...
public struct MockQuotaProvider: QuotaProviding {
    ...
    /// The account the sample readings belong to — what titles the quota
    /// section in previews. `email` is what ``QuotaAccount/displayName``
    /// actually renders.
    public static func sampleAccount() -> QuotaAccount {
        QuotaAccount(
