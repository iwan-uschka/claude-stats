...
- A quota bar no longer keeps showing its old percentage after that window has
  ...
  source. If both windows have reset and no statusline hook is installed, the
  popover shows the "no quota source" error instead of a stale reading.
...
- The periodic CPU spike is roughly halved, and grows far more slowly with the
  size of the log corpus. A write now costs a look at the files that changed
  plus an in-memory sweep of the rest, instead of a full stat-and-enumerate
  walk over every session file on disk. The full walk still runs at launch,
  and whenever the system reports it lost track of what changed.
